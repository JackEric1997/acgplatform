...
import com.acg.authority.dao.RoleDao;
...
public class RoleServiceImpl implements RoleService {
    ...
    public Map<String, Object> findRoleByUserId(String userId) {
        //查询所有的角色
        List<Role> allRolesList = roleDao.allRolesList();

        //根据用户id，查询用户拥有的角色id
        List<UserRole> existUserRoleList = userRoleDao.existUserRoleList(userId);
...
/*        List<UserRole> userRoleList = userRoleService.list(new QueryWrapper<UserRole>().eq("user_id", id).select("role_id"));
        ...
        */
        //根据用户id拥有的角色id
        List<UserRole> userRoleList = userRoleService.existUserRoleList(id);
        List<String> roleIdList = userRoleList.stream().map(item -> item.getRoleId()).collect(Collectors.toList());
        //根据角色id查用户
        List<Role> roleList = new ArrayList<>();
        if(roleIdList.size() > 0) {
            roleList = roleDao.existRoleList(roleIdList);
        }
        return roleList;
    }
}
